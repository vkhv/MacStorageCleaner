...
struct SystemAnalysisView: View {
    ...
    private func performAnalysis() async {
        ...
        // Асинхронно анализируем папку
        let result = await withTaskGroup(of: SystemFolderAnalysis?.self) { group in
            group.addTask {
                return await SystemFolderAnalyzer.analyzeSystemFolderAsync(directory.path, dirInfo: directory)
            }
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 12) {
                        ...
                        Button(action: {
                            NSWorkspace.shared.selectFile(nil, inFileViewerRootedAtPath: recommendation.title)
                        }) {
